...
前些阵子些许无聊，借此空闲时间想用 Canvas 实现一个小游戏，选来选去最后决定是 2048（太难容易没巩固的效果，太难打击自信心 🤣）。但让我没想到的是其中 Canvas 涉及的东西并不多。下面补充一下 2048 的游戏规则[^2048-rules-answer]：

一、开始时随机出现一个格子。

二、手指向一个方向滑动，所有格子会向那个方向运动。

三、相同数字的两个格子，相撞时数字会相加（格子合并），且只可出现一次**同时**合并。

四、每次滑动时，空白处会随机刷新出一个数字的格子，值为 2 或 4。

五、当界面不可运动时（当界面全部被数字填满时），游戏结束；当界面中最大数字是 2048 时，游戏胜利。
...
参考最火热的 2048 在线试玩[网站](https://play2048.co/)，决定实现如下效果[^2]：
...
<CenterImg src="https://res.zrain.fun/images/2022/04/%E5%AE%9E%E7%8E%B0%E5%9C%86%E8%A7%92%E7%9F%A9%E5%BD%A21-bf151486f0d4cdb071f02d996807f0c1.png" alt="实现圆角矩形1" zoom="60%"/>
...
[^2048-rules-answer]: [2048 的游戏规则是什么？ - 百度百科](https://zhidao.baidu.com/question/1994394486771253027/answer/3040502574)
[^2]: [canvas 绘制 2048 游戏的背景 - CSDN](https://blog.csdn.net/fatansitic/article/details/51981298)
